...
// Generative CPPN shader for civic engagement theme
const vertexShader = `
  varying vec2 vUv;
  void main() {
    vUv = uv;
...
const fragmentShader = `
  ...
  void main() {
    ...
    gl_FragColor = cppn_fn(uv, 0.1 * sin(0.35 * iTime + 1.2), 0.1 * sin(0.73 * iTime + 2.1), 0.1 * sin(0.52 * iTime + 0.8));
  }
`;
...
function ShaderPlane() {
  ...
  useFrame((state) => {
    ...
    (materialRef.current as any).iTime = state.clock.elapsedTime;
    const { width, height } = state.size;
    (materialRef.current as any).iResolution.set(width, height);
  });
...
export function ShaderBackground() {
  ...
  return (
    <div ref={canvasRef} className="absolute inset-0 -z-10 w-full h-full bg-gradient-to-br from-civic-blue to-civic-green" aria-hidden>
      <Canvas
        camera={camera}
